...
rateFunctions2<-list(
    ...
    reproFunc=function(x,time,species,strain){
        ...
        if (species==3){#H eggs eaten by M
            if (time>=timeExposureSt & time<=timeExposureEnd){
                reprod=alphaM*sum(x$Hoppers['Eggs',])/(sum(x$Parasitoids['Eggs',])+sum(x$Hoppers['Eggs',])+Km)
            }
        }
        
        if (species==4){#P eggs eaten by M
            if (time>=timeExposureSt & time<=timeExposureEnd){
                reprod=alphaM*sum(x$Parasitoids['Eggs',])/(sum(x$Parasitoids['Eggs',])+sum(x$Hoppers['Eggs',])+Km)
            }
        }
